...
import telegramData, { myStory } from "./telegram-data";
...
const Avatar = forwardRef<any, AvatarProps>(({ bg, image, size, hasStories, style }, ref) => {
  const borderSize = 2;
...
function MessageListItem({ item }: MessageListItemProps) {
  ...
  return (
    <TouchableOpacity onPress={() => alert(`Pressed on ${item.name}`)}>
      ...
      >
        <Avatar image={item.avatar} size={60} hasStories={item.hasStories} bg={item.bg} />
        <View
          style={{
            borderBottomWidth: 1,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Animated.Text style={[{ fontWeight: "800" }, headerStylez]}>Chats</Animated.Text>
      </View>
...
function AnimatedStoryItem({
  ...
  const stylez = useAnimatedStyle(() => {
    ...
    return {
      ...
      width: interpolate(animatedIndex.value, [0, 1], [storyAvatarSize, measurement.width]),
      height: interpolate(animatedIndex.value, [0, 1], [storyAvatarSize, measurement.height]),
      transform: [
        {
          translateY: interpolate(
            animatedIndex.value,
...
function ExpandedList<T>({
  ...
  return (
    <Animated.ScrollView
      horizontal
      style={[{ flexGrow: 0, overflow: "visible", backgroundColor: "transparent" }, stylez]}
...
export function TelegramStories({ data }: MessageListProps) {
  ...
  const refs = data
    .filter((item) => item.hasStories)
    .slice(0, maxHeaderItems)
    .reduce((acc, item) => ({ ...acc, [item.key]: useAnimatedRef() }), {} as ActiveItemRefs);
    ...
    <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
      ...
      */}
      ...
      <BottomSheet
        ...
        snapPoints={[height - headerHeight - storiesHeight - top, height - headerHeight - top]}
      >
